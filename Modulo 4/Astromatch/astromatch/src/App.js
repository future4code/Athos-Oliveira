...
import BotaoSwipe from "./components/Botao";
...
const App = () => {
  ...
  const exibirCrush = async () => {
    try {
      setloading(true)
      const response = await axios.get("https://us-central1-missao-newton.cloudfunctions.net/astroMatch/athos-joy/person")
  ...
  const selecionarPessoa = (id) => {
    axios
      .post(
        "https://us-central1-missao-newton.cloudfunctions.net/astroMatch/athos-joy/choose-person",
        {
          id: pessoa.id,
          choice: true,
        }
  ...
  const irTelaMatch = async () => {
    try {
      const response = await axios.get(
        "https://us-central1-missao-newton.cloudfunctions.net/astroMatch/athos-joy/matches"
      );
    ...
    if (confirm("Voce deseja reiniciar os seus Matches?")) {
      try {
        const res = await axios.put(
          "https://us-central1-missao-newton.cloudfunctions.net/astroMatch/athos-joy/clear"
        );
        setAtualizador(!atualizador);
